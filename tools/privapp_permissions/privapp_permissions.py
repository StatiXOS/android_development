...
class Resources(object):
    ...
    def _resolve_all_system_privapps(self):
        ...
        if self._is_android_env:
            system_priv_app_dir = os.path.join(os.environ['ANDROID_PRODUCT_OUT'],
                                        'system/priv-app')
        else:
            try:
    ...
    def _resolve_all_product_privapps(self):
        ...
        if self._is_android_env:
            ...
            try:
                ...
                print('Directory "/product/priv-app" could not be pulled from on '
                    'device "%s". Trying "/system/product/priv-app"' % self.adb.serial)
                try:
                    product_priv_app_dir = self.adb.pull('/system/product/priv-app/')
                except subprocess.CalledProcessError:
                    raise MissingResourceError(
    ...
    def _resolve_sys_path(self, file_path, fallback_file_path=None):
        ...
        if self._is_android_env:
            sys_path = os.path.join(os.environ['ANDROID_PRODUCT_OUT'], file_path)
            if not os.path.exists(sys_path):
                sys_path = os.path.join(os.environ['ANDROID_PRODUCT_OUT'], fallback_file_path)
        else:
            try:
                sys_path = self.adb.pull(file_path)
            except subprocess.CalledProcessError:
                print('Directory %s could not be pulled from on '
                    'device "%s". Trying "/system/product/priv-app"' % file_path, self.adb.serial)
                try:
...
def create_permission_file(resources, privapp_apks, permissions_dir, sysconfig_dir):
